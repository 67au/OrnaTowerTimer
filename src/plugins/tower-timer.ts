...
const kinds: Array<string> = ["selene", "eos", "oceanus", "themis", "prometheus"];
// 45, 40, 35, 30, 25
 ...
 * @param { Date } time
 */
export function getTowerFloors(time: Date) {
...
const checkpoints: Array<[number, number]> = [[0, 0], [1, 0], [5, 0], [10, 0], [15, 0], [15, 36], [20, 0]];
...
export function getTowerFloorsInNextDays(time: Date, n: number = 2) {
	...
	return Array.from(days).map((timeTuple: [number, number], index: number) => {
